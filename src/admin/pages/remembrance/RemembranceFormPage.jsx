...
export default function RemembranceFormPage() {
  const { id } = useParams()
  ...
  const handleSubmit = async (fields, imageFile, gallerySlots) => {
    ...
    try {
      if (isEdit) {
        await updateRemembrancePost(id, fields, imageFile || undefined, gallerySlots)
      } else {
        await createRemembrancePost(fields, imageFile, gallerySlots)
      }
      invalidateRemembranceCache()
      navigate('/admin/remembrance', { replace: true })
    } catch (err) {
  ...
  const goBack = () => navigate('/admin/remembrance')
