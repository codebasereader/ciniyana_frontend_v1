...
export default function PosterListPage() {
  ...
  const handleDelete = async (post) => {
    ...
    try {
      await deletePosterPost(post.id)
  ...
  const handleSaveOrder = async (orderedIds) => {
    setSavingOrder(true)
    try {
      const updated = await reorderPosterPosts(orderedIds)
      setPosts(updated)
      invalidatePosterCache()
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-[#ac222b] sm:text-3xl">
            {isKn ? 'ಪೋಸ್ಟರ್ ಮಾಹಿತಿ' : 'Poster Info'}
          </h1>
          <p className="mt-1 text-sm text-[#666]">
            {isKn ? 'ಚಿತ್ರ + ಕನ್ನಡ ಮತ್ತು ಇಂಗ್ಲಿಷ್ ಶೀರ್ಷಿಕೆ' : 'Image + English and Kannada title'}
          </p>
        </div>
      ...
      {!loading && !error && posts.length === 0 ? (
        <p className="rounded-xl border border-[#ead9c4] bg-[#fbf6ef] px-4 py-6 text-sm text-[#555]">
          {isKn
            ...
            : 'No posts yet. Create the first poster.'}
        </p>
      ) : null}
      ...
      <ul className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {visible.map((post) => {
          ...
          return (
            ...
            >
              <div className="aspect-[3/4] bg-[#ece8e1]">
                {post.image ? (
                  <img
                    src={post.image}
                    alt={title}
              ...
              <div className="p-3">
                ...
                <p className="mt-1 line-clamp-1 text-xs text-[#777]">
                  {language === 'en' ? post.title?.kn : post.title?.en}
                </p>
                <div className="mt-3 flex gap-2">
                  <Link
                    to={`/admin/poster/${post.id}/edit`}
