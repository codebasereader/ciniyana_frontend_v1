...
export default function FlashBackListPage() {
  ...
  const handleDelete = async (post) => {
    ...
    try {
      await deleteFlashBackPost(post.id)
  ...
  const handleSaveOrder = async (orderedIds) => {
    setSavingOrder(true)
    try {
      const updated = await reorderFlashBackPosts(orderedIds)
      setPosts(updated)
      invalidateFlashBackCache()
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-[#ac222b] sm:text-3xl">
            {isKn ? 'ಫ್ಲಾಷ್‌ಬ್ಯಾಕ್‌' : 'Flash Back'}
          </h1>
          <p className="mt-1 text-sm text-[#666]">
            {isKn ? 'ಪೋಸ್ಟ್‌ಗಳನ್ನು ನಿರ್ವಹಿಸಿ' : 'Manage posts (EN + KN content)'}
          </p>
        </div>
      ...
      {!loading && !error && posts.length === 0 ? (
        <p className="rounded-xl border border-[#ead9c4] bg-[#fbf6ef] px-4 py-6 text-sm text-[#555]">
          {isKn
            ...
            : 'No posts yet. Create the first Flash Back post.'}
        </p>
      ) : null}
            ...
            >
              ...
              <div className="p-3">
                ...
                <div className="mt-3 flex gap-2">
                  <Link
                    to={`/admin/flash-back/${post.id}/edit`}
