...
export default function VideoListPage() {
  ...
  const handleSaveOrder = async (orderedIds) => {
    setSavingOrder(true)
    try {
      const updated = await reorderVideoPosts(orderedIds)
      setPosts(updated)
      invalidateVideoCache()
    ...
    <div className="mx-auto max-w-5xl">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-[#666]">
            {isKn
              ? 'YouTube ಲಿಂಕ್ + ಕನ್ನಡ ಮತ್ತು ಇಂಗ್ಲಿಷ್ ಶೀರ್ಷಿಕೆ'
              : 'YouTube link + English and Kannada title'}
          </p>
        </div>
      ...
      {!loading && !error && posts.length === 0 ? (
        <p className="rounded-xl border border-[#ead9c4] bg-[#fbf6ef] px-4 py-6 text-sm text-[#555]">
          {isKn
            ? 'ಯಾವುದೇ ವೀಡಿಯೋ ಇಲ್ಲ. ಹೊಸದನ್ನು ಸೇರಿಸಿ.'
      ...
      {!loading && posts.length > 0 && filtered.length === 0 ? (
        <p className="rounded-xl border border-[#ead9c4] bg-[#fbf6ef] px-4 py-6 text-sm text-[#555]">
          {isKn ? 'ಯಾವುದೇ ಫಲಿತಾಂಶ ಸಿಗಲಿಲ್ಲ.' : 'No results match your search.'}
        </p>
      ) : null}
