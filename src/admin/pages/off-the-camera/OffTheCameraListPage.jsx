...
import { invalidateOffTheCameraCache } from '../../../pages/off-the-camera/useOffTheCameraPosts'
...
export default function OffTheCameraListPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError('')
    try {
      const list = await fetchOffTheCameraPosts()
      setPosts(list)
  ...
  const handleDelete = async (post) => {
    ...
    try {
      await deleteOffTheCameraPost(post.id)
      invalidateOffTheCameraCache()
  ...
  const handleSaveOrder = async (orderedIds) => {
    setSavingOrder(true)
    try {
      const updated = await reorderOffTheCameraPosts(orderedIds)
      setPosts(updated)
      invalidateOffTheCameraCache()
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-[#ac222b] sm:text-3xl">
            {isKn ? 'ಆಫ್‌ ದಿ ಕ್ಯಾಮೆರಾ' : 'Off the Camera'}
          </h1>
          <p className="mt-1 text-sm text-[#666]">
            {isKn
              ? 'ಪೋಸ್ಟ್‌ಗಳನ್ನು ನಿರ್ವಹಿಸಿ'
              : 'Manage posts (EN + KN + subtitle + profile)'}
          </p>
        </div>
      ...
      {!loading && !error && posts.length === 0 ? (
        <p className="rounded-xl border border-[#ead9c4] bg-[#fbf6ef] px-4 py-6 text-sm text-[#555]">
          {isKn
            ...
            : 'No posts yet. Create the first Off the Camera post.'}
        </p>
      ) : null}
            ...
            >
              ...
              <div className="p-3">
                ...
                <div className="mt-3 flex gap-2">
                  <Link
                    to={`/admin/off-the-camera/${post.id}/edit`}
