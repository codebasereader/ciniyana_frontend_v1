...
import { invalidateInfoSpecialCache } from '../../../pages/info-special/useInfoSpecialPosts'
...
export default function InfoSpecialListPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError('')
    try {
      const list = await fetchInfoSpecialPosts()
      setPosts(list)
  ...
  const handleDelete = async (post) => {
    ...
    try {
      await deleteInfoSpecialPost(post.id)
      invalidateInfoSpecialCache()
  ...
  const handleSaveOrder = async (orderedIds) => {
    setSavingOrder(true)
    try {
      const updated = await reorderInfoSpecialPosts(orderedIds)
      setPosts(updated)
      invalidateInfoSpecialCache()
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-[#ac222b] sm:text-3xl">
            {isKn ? 'ಮಾಹಿತಿ ವಿಶೇಷ' : 'Info Special'}
          </h1>
          <p className="mt-1 text-sm text-[#666]">
            {isKn
              ? 'ಪೋಸ್ಟ್‌ಗಳನ್ನು ನಿರ್ವಹಿಸಿ'
              : 'Manage posts (EN + KN + gallery + profile)'}
          </p>
        </div>
      ...
      {!loading && !error && posts.length === 0 ? (
        <p className="rounded-xl border border-[#ead9c4] bg-[#fbf6ef] px-4 py-6 text-sm text-[#555]">
          {isKn
            ...
            : 'No posts yet. Create the first Info Special post.'}
        </p>
      ) : null}
            ...
            >
              ...
              <div className="p-3">
                ...
                <div className="mt-3 flex gap-2">
                  <Link
                    to={`/admin/info-special/${post.id}/edit`}
