...
export default function FilmTodayListPage() {
  ...
  const handleDelete = async (post) => {
    ...
    try {
      await deleteFilmTodayPost(post.id)
  ...
  const handleSaveOrder = async (orderedIds) => {
    setSavingOrder(true)
    try {
      const updated = await reorderFilmTodayPosts(orderedIds)
      setPosts(updated)
      invalidateFilmTodayCache()
      ...
      {!loading && !error && posts.length === 0 ? (
        <p className="rounded-xl border border-[#ead9c4] bg-[#fbf6ef] px-4 py-6 text-sm text-[#555]">
          {isKn
            ...
            : 'No posts yet. Create the first Photo Story post.'}
        </p>
      ) : null}
            ...
            >
              ...
              <div className="p-3">
                ...
                <div className="mt-3 flex gap-2">
                  <Link
                    to={`/admin/film-today/${post.id}/edit`}
