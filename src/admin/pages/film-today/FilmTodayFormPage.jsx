...
import { invalidateFilmTodayCache } from '../../../pages/film-today/useFilmTodayPosts'
import { selectLanguage } from '../../../store/slices/languageSlice'
import FilmTodayForm from './components/FilmTodayForm'
import { galleryFromPost, postToFormFields } from './utils'
import FormPageHeader from '../../components/FormPageHeader'

export default function FilmTodayFormPage() {
  const { id } = useParams()
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError('')
      try {
        const list = await fetchFilmTodayPosts()
  ...
  const handleSubmit = async (fields, imageFile, gallerySlots) => {
    ...
    try {
      if (isEdit) {
        await updateFilmTodayPost(id, fields, imageFile || undefined, gallerySlots)
      } else {
        await createFilmTodayPost(fields, imageFile, gallerySlots)
      }
      invalidateFilmTodayCache()
      navigate('/admin/film-today', { replace: true })
    } catch (err) {
  ...
  const goBack = () => navigate('/admin/film-today')
