...
export default function ArticleListPage() {
  ...
  const handleDelete = async (post) => {
    ...
    try {
      await deleteArticlePost(post.id)
  ...
  const handleSaveOrder = async (orderedIds) => {
    setSavingOrder(true)
    try {
      const updated = await reorderArticlePosts(orderedIds)
      setPosts(updated)
      invalidateArticleCache()
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-[#666]">
            {isKn
              ...
              : 'Manage posts (EN + KN + subtitle + profile + interleave gallery)'}
          </p>
        </div>
      ...
      {!loading && !error && posts.length === 0 ? (
        <p className="rounded-xl border border-[#ead9c4] bg-[#fbf6ef] px-4 py-6 text-sm text-[#555]">
          {isKn
            ...
            : 'No posts yet. Create the first Article post.'}
        </p>
      ) : null}
