...
export async function fetchFilmTodayPosts() {
  const data = await apiFetch('/film-today', { skipAuth: true })
  return withResolvedImages(data.posts || [])
}

export async function fetchFilmTodayBySlug(slug) {
  const data = await apiFetch(`/film-today/${encodeURIComponent(slug)}`, {
    skipAuth: true,
  })
  return {
...
export async function createFilmTodayPost(fields, imageFile, gallerySlots = []) {
  const data = await apiFetch('/film-today', {
    method: 'POST',
    body: buildFilmTodayFormData(fields, imageFile, gallerySlots),
  })
  return withResolvedImage(data.post)
}

export async function updateFilmTodayPost(id, fields, imageFile, gallerySlots = []) {
  const data = await apiFetch(`/film-today/${encodeURIComponent(id)}`, {
    method: 'PUT',
    body: buildFilmTodayFormData(fields, imageFile, gallerySlots),
  })
  return withResolvedImage(data.post)
...
export async function deleteFilmTodayPost(id) {
  return apiFetch(`/film-today/${encodeURIComponent(id)}`, {
    method: 'DELETE',
  })
}

export async function reorderFilmTodayPosts(orderedIds) {
  const data = await apiFetch('/film-today/reorder', {
    method: 'PUT',
