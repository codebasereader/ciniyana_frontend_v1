...
import { fetchFilmTodayPosts } from '../../api'
import { filmTodayPosts as staticPosts } from './posts'
...
export function invalidateFilmTodayCache() {
  cache = { posts: null, promise: null, error: null }
}

async function loadFilmTodayPosts({ fallback = true } = {}) {
  if (cache.posts) {
    return { posts: cache.posts, fromApi: true, error: null }
  }

  if (!cache.promise) {
    cache.promise = fetchFilmTodayPosts()
      .then((posts) => {
        cache.posts = posts
        cache.error = null
        return posts
...
export function useFilmTodayPosts({ fallback = true } = {}) {
  ...
  const reload = useCallback(async () => {
    ...
    setLoading(true)
    const result = await loadFilmTodayPosts({ fallback })
