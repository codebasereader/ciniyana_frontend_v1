...
import { fetchOffTheCameraPosts } from '../../api'
import { offTheCameraPosts as staticPosts } from './posts'
...
export function invalidateOffTheCameraCache() {
  cache = { posts: null, promise: null, error: null }
}

async function loadOffTheCameraPosts({ fallback = true } = {}) {
  if (cache.posts) {
    return { posts: cache.posts, fromApi: true, error: null }
  }

  if (!cache.promise) {
    cache.promise = fetchOffTheCameraPosts()
      .then((posts) => {
        cache.posts = posts
        cache.error = null
        return posts
...
export function useOffTheCameraPosts({ fallback = true } = {}) {
  ...
  const reload = useCallback(async () => {
    ...
    setLoading(true)
    const result = await loadOffTheCameraPosts({ fallback })
